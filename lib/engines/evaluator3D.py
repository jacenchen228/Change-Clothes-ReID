...
class Evaluator3D(object):
    ...
    def evaluate(self):
        ...
        for batch_idx, data in enumerate(self.queryloader):
            ...
            # concate imgs and segments
            # inputs_concate = torch.cat([imgs, segments], dim=1)

            # features_list = self._extract_features(segments, imgs)
        ...
        for batch_idx, data in enumerate(self.galleryloader):
            ...
            features_list = self._extract_features(imgs, inputs3d)
            # features_list = self._extract_features(imgs, imgs)
            # features_list = self._extract_features(imgs, inputs_concate)
        ...
        for i, (qf, gf) in enumerate(zip(qf_list, gf_list)):
            ...
            # distmat_ensemble = torch.load('/home/jiaxing/MyReID/ensemble_distmats/ensemble_distmat.pt')
            # distmat_ensemble = (distmat_ensemble - torch.min(distmat_ensemble)) / (torch.max(distmat_ensemble) - torch.min(distmat_ensemble))
            # distmat = (distmat - torch.min(distmat)) / (torch.max(distmat) - torch.min(distmat))
            # distmat = 0.6 * distmat + distmat_ensemble
    ...
    def _extract_features(self, imgs, input3d):
        self.model.eval()

        return self.model(imgs, input3d)
