...
class MyModel(nn.Module):
    ...
    def featuremaps(self, x1, x2):
        ...
        # adj_mat = self.cos_sim(x2_part_new)
        # adj_mat = self.normalize(adj_mat)
        # x2_part_new = self.gnns1[-1](x2_part_new, adj_mat)
        # x2_part_new = self.bns_gnn1[-1](x2_part_new.transpose(1, 2))
        # x2_part_new = x2_part_new.transpose(1, 2)
        # x2_part_new = self.relu(x2_part_new)
        #
        # adj_mat = self.cos_sim(x2_part_new)
        # adj_mat = self.normalize(adj_mat)
        # x2_part_new = self.gnns2[-1](x2_part_new, adj_mat)
        # x2_part_new = self.bns_gnn2[-1](x2_part_new.transpose(1, 2))
        # x2_part_new = x2_part_new.transpose(1, 2)
        # x2_part_new = self.relu(x2_part_new)

        # x2_part = x2_part.transpose(1, 2)
        # x2_part = x2_part.view(x2_part.size(0), x2_part.size(1), -1)
        # adj_mat = self.cos_sim(x2_part)
        # adj_mat = self.normalize(adj_mat)
        # x2_part_new = self.gnn(x2_part, adj_mat)
        # x2_part_new = self.bn_gnn(x2_part_new.transpose(1, 2))
        # x2_part_new = x2_part_new.transpose(1, 2)
        # x2_part_new = self.relu(x2_part_new)
    ...
    def forward(self, x1, x2, return_featuremaps=False):
        ...
        if self.loss == 'softmax':
            return y1, y1_parts, y2, y_fuse, ej, em, ej_part, em_part
...
"""ResNet"""
def dim_part_graph_hie_abla(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = MyModel(
