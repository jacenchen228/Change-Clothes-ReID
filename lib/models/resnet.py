...
class MyModel(nn.Module):
    ...
    def __init__(self, num_classes, block_rgb, layers_rgb, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, last_stride=2, fc_dims=None, dropout_p=None, reduce_dim=768, **kwargs):
        ...
        # self.bnneck = nn.BatchNorm1d(self.feature_dim) # bnneck layers
    ...
    def forward(self, x1, x2=None, return_featuremaps=False):
        f = self.featuremaps(x1)
        ...
        if not self.training:
            return [v, v_bnneck, torch.cat([v, v_bnneck], dim=1)]
...
def resnet50(num_classes=150, loss='softmax', pretrained=True, **kwargs):
    model = MyModel(
