...
from lib.lib3D import SMPL, batch_rodrigues, Discriminator
from lib.lib3D.hmr_new3 import hmr_new
...
class Model_3D_2stream(nn.Module):
    def __init__(self, num_classes, batch_size, reduce_dim=768, norm_layer=None, **kwargs):
        ...
        self.discriminator = Discriminator(feature_dim=512*block.expansion)
    ...
    def forward(self, inputs1, inputs2, real_params=None, return_params3D=False):
        ...
        f3d, pred_rotmats, pred_betas, pred_cam = self.estimator3D(inputs2)
        pred_params = {'rotmat':pred_rotmats,
                       'beta':pred_betas,
                       'cam':pred_cam}

        pred_outputs = self.smpl(betas=pred_betas, body_pose =pred_rotmats[:,1:],
                                 global_orient=pred_rotmats[:,0].unsqueeze(1), pose2rot=False)

        if return_params3D:
            return pred_rotmats, pred_betas, pred_cam, pred_outputs
        ...
        if not self.training:
            v_concate1 = torch.cat([F.normalize(v, p=2, dim=1), F.normalize(v3d, p=2, dim=1)], dim=1)
            v_concate2 = torch.cat([F.normalize(v_bnneck, p=2, dim=1), F.normalize(v3d_bnneck, p=2, dim=1)], dim=1)

            return [v, v_bnneck, v3d, v3d_bnneck, v_concate1, v_concate2]

        y = self.classifier_reid(v_bnneck)
        ...
        return [y, y3d], [v, v3d], pred_params, encoder_disc_value, gen_disc_value, real_disc_value, pred_outputs
...
def model_3D_2stream_v4(num_classes=150, loss='softmax', **kwargs):
