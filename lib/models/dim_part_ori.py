...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes, loss, block, layers, block_aux, layers_aux, zero_init_residual=False,
        ...
        self.loss = loss
        self.feature_dim_base = 256
        self.feature_dim = self.feature_dim_base * block.expansion
        self.feature_dim_aux = self.feature_dim_base * block_aux.expansion
        self.inplanes = 64
        self.dilation = 1
        self.part_num = 3
        ...
        self.layer4 = self._make_layer(block, self.feature_dim_base, layers[3], stride=last_stride,
                                       dilate=replace_stride_with_dilation[2])
        self.inplanes = 256 * block.expansion
        self.layer4_part = self._make_layer(block, self.feature_dim_base, layers[3], stride=last_stride,
                                       dilate=replace_stride_with_dilation[2])
        ...
        self.layer4_aux = self._make_layer(block_aux, self.feature_dim_base, layers_aux[3], stride=last_stride)
        ...
        self.feature_dim_gcn = self.feature_dim_base * block.expansion
        ...
        # self.gcn2 = GraphConvolution(self.feature_dim_gcn, self.feature_dim_gcn)
        ...
        self.discriminator = GlobalDiscriminator(16, 8, in_feature_dim=self.feature_dim_base*block.expansion,
                                                 feature_dim=256)
    ...
    def featuremaps(self, x, x_aux):
        ...
        # adj1 = self.cos_sim(x_part)
        # x_part = self.gcn1(x_part, adj1)
        #
        # adj2 = self.cos_sim(x_part)
        # x_part = self.gcn2(x_part, adj2)
    ...
    def forward(self, x1, x2, return_featuremaps=False):
        ...
        f, f_aux, fpart, fpart_fuse = self.featuremaps(x, x_aux)
        ...
        if not self.training:
            ...
            # v_aux = F.normalize(v_aux, p=2, dim=1)
            # v = F.normalize(v, p=2, dim=1)
            # v_fuse = F.normalize(v_fuse, p=2, dim=1)
            # vpart = F.normalize(vpart, p=2, dim=1)
            # vpart_fuse = F.normalize(vpart_fuse, p=2, dim=1)
            ...
            # test_feat1 = v_aux
            # test_feat2 = v_fuse
            # test_feat3 = torch.cat([v_fuse, v_aux], dim=1)
            # test_feat4 = torch.max(fpart, dim=1)[0]
            # test_feat5 = torch.max(fpart_fuse, dim=1)[0]
            # test_feat6 = torch.cat([v_fuse, v_aux, test_feat4], dim=1)
            # test_feat7 = torch.cat([v_fuse, v_aux, test_feat5], dim=1)
            # test_feat8 = torch.cat([v_fuse, v_aux, test_feat4, test_feat5], dim=1)

            # return [test_feat0, test_feat1, test_feat2, test_feat3, test_feat4, test_feat5, test_feat6, test_feat7, test_feat8]

            ############## test feature ##############

            v_aux = F.normalize(v_aux, p=2, dim=1)
            test_feat0 = F.normalize(v, p=2, dim=1)
            test_feat1 = F.normalize(v_fuse, p=2, dim=1)
            test_feat2 = F.normalize(vpart, p=2, dim=1)
            test_feat3 = F.normalize(vpart_fuse, p=2, dim=1)
            test_feat4 = torch.cat([test_feat0, test_feat1], dim=1)
            test_feat5 = torch.cat([test_feat2, test_feat3], dim=1)
            test_feat6 = torch.cat([test_feat0, test_feat1, test_feat2, test_feat3], dim=1)
            test_feat7 = torch.cat([test_feat0, test_feat1, v_aux], dim=1)
            test_feat8 = torch.cat([test_feat2, test_feat3, v_aux], dim=1)
            test_feat9 = torch.cat([test_feat0, test_feat1, test_feat2, test_feat3, v_aux], dim=1)
            test_feat10 = torch.cat([test_feat0, test_feat2], dim=1)
            test_feat11 = torch.cat([test_feat1, test_feat3], dim=1)
            test_feat12 = torch.cat([test_feat1, test_feat3, v_aux], dim=1)
            test_feat13 = torch.cat([test_feat1, v_aux], dim=1)

            return [test_feat0, test_feat1, test_feat2, test_feat3, test_feat4, test_feat5,
                    test_feat6, test_feat7, test_feat8, test_feat9, test_feat10, test_feat11, test_feat12, test_feat13, v_aux]
        ...
        if self.loss == 'softmax':
            ...
            # return y, y_aux, y_fuse, ypart, ypart_fuse
            # return y, y_aux, y_fuse, ej, em, ypart, ypart_fuse
            # return y, y_aux, y_fuse, ej_part, em_part, ypart, ypart_fuse
        elif self.loss == 'triplet':
            return y, y_aux, y_fuse, ej, em, ej_part, em_part, ypart, ypart_fuse, \
                   v, v_aux, v_fuse, vpart, vpart_fuse
            # return y, y_aux, y_fuse, ypart, ypart_fuse, v, v_aux, v_fuse, vpart, vpart_fuse
            # return y, y_aux, y_fuse, ej, em, ypart, ypart_fuse, \
            #        v, v_aux, v_fuse, vpart, vpart_fuse
            # return y, y_aux, y_fuse, ej_part, em_part, ypart, ypart_fuse, \
            #        v, v_aux, v_fuse, vpart, vpart_fuse
