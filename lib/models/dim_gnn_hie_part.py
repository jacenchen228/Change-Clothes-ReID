...
__all__ = ['dim_part_graph_hie_resnet50_C']
...
class MyModel(nn.Module):
    ...
    def __init__(self, num_classes, loss, block_rgb, layers_rgb, block_contour, layers_contour, zero_init_residual=False,
        ...
        self.classifiers_part = nn.ModuleList([nn.Linear(self.reduce_dim, num_classes) for _ in range(self.part_num_rgb)])
        ...
        #                                          feature_dim=1024)
        self.discriminator_global = GlobalDiscriminator(in_feature_dim=2048, feature_dim=512)
        self.part_height = 16 // self.part_num_rgb
        self.residue = 16 % self.part_num_rgb
        self.discriminator_part = PartDiscriminator(in_feature_dim=2048, feature_dim=512, vec_feature_dim=self.reduce_dim, part_height=self.part_height)
    ...
    def forward(self, x1, x2, return_featuremaps=False):
        ...
        if self.loss == 'softmax':
            ...
            return y1, y1_parts, y2, y_fuse, v1, v2, v_fuse, ej, em, ej_part, em_part
        else:
...
"""ResNet"""
def dim_part_graph_hie_resnet50_C(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = MyModel(
