...
class MyModel(nn.Module):
    ...
    def __init__(self, num_classes, loss, block_rgb, layers_rgb, block_contour, layers_contour, zero_init_residual=False,
        ...
        # self.gnn = GraphConvolution(self.feature_dim, self.feature_dim_gnn)
        # self.bn_gnn = nn.BatchNorm1d(self.feature_dim_gnn)
    ...
    def featuremaps(self, x1, x2):
        ...
        x2 = self.parts_avgpool_contour(x2)

        # fine-grained scale graph learning
        x2 = x2.permute(0, 2, 3, 1)
        x2_new = torch.zeros(x2.shape[0], x2.shape[1], x2.shape[3]).cuda()
        for idx in range(x2.shape[1]):
            ...
            # adj_mat_i = self.normalize(adj_mat_i)
            # part_i_new = self.gnns1[idx](part_i, adj_mat_i)
            # part_i_new = self.bns_gnn1[idx](part_i_new.transpose(1, 2))
            # part_i_new = part_i_new.transpose(1, 2)
            # part_i_new = self.relu(part_i_new)
            #
            # adj_mat_i = self.cos_sim(part_i_new)
            # adj_mat_i = self.normalize(adj_mat_i)
            # part_i_new = self.gnns2[idx](part_i, adj_mat_i)
            # part_i_new = self.bns_gnn2[idx](part_i_new.transpose(1, 2))
            # part_i_new = part_i_new.transpose(1, 2)
            # part_i_new = self.relu(part_i_new)

            x2_new[:, idx, :] = torch.max(part_i_new, dim=1)[0]

        # coarse-grained scale graph learning
        adj_mat = self.cos_sim(x2_new)
        adj_mat = self.normalize(adj_mat)
        x2_new = self.gnns[-1](x2_new, adj_mat)
        x2_new = self.bns_gnn[-1](x2_new.transpose(1, 2))
        x2_new = x2_new.transpose(1, 2)
        x2_new = self.relu(x2_new)

        # adj_mat = self.cos_sim(x2_new)
        # adj_mat = self.normalize(adj_mat)
        # x2_new = self.gnns1[-1](x2_new, adj_mat)
        # x2_new = self.bns_gnn1[-1](x2_new.transpose(1, 2))
        # x2_new = x2_new.transpose(1, 2)
        # x2_new = self.relu(x2_new)
        #
        # adj_mat = self.cos_sim(x2_new)
        # adj_mat = self.normalize(adj_mat)
        # x2_new = self.gnns2[-1](x2_new, adj_mat)
        # x2_new = self.bns_gnn2[-1](x2_new.transpose(1, 2))
        # x2_new = x2_new.transpose(1, 2)
        # x2_new = self.relu(x2_new)
        
        # x2 = x2.transpose(1, 2)
        # x2 = x2.view(x2.size(0), x2.size(1), -1)
        # adj_mat = self.cos_sim(x2)
        # adj_mat = self.normalize(adj_mat)
        # x2_new = self.gnn(x2, adj_mat)
        # x2_new = self.bn_gnn(x2_new.transpose(1, 2))
        # x2_new = x2_new.transpose(1, 2)
        # x2_new = self.relu(x2_new)

        return x1, x2_new, x_fuse
...
"""ResNet"""
def my_baseline_gnn_hie(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = MyModel(
