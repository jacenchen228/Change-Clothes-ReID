...
__all__ = ['dim_multigcn_multi50', 'dim_multigcn_multi34']
...
class MyModel(nn.Module):
    ...
    def __init__(self, num_classes, loss, block_rgb, layers_rgb, block_contour, layers_contour, zero_init_residual=False,
        ...
        self.contour_gnns1 = []
        self.contour_gnn_bns1 = []
        self.contour_gnns2 = []
        self.contour_gnn_bns2 = []
        for idx in range(self.layer_num):
            ...
            gnns1 = nn.ModuleList([GraphConvolution(base_dim*block_contour.expansion, base_dim*block_contour.expansion, bias=True)
                                       for _ in range(self.part_num + 1)])
            bns_gnn1 = nn.ModuleList([nn.BatchNorm1d(base_dim*block_contour.expansion) for _ in range(self.part_num + 1)])

            gnns2 = nn.ModuleList([GraphConvolution(base_dim*block_contour.expansion, base_dim*block_contour.expansion, bias=True)
                                       for _ in range(self.part_num + 1)])
            bns_gnn2 = nn.ModuleList([nn.BatchNorm1d(base_dim*block_contour.expansion) for _ in range(self.part_num + 1)])

            self.contour_gnns.append(gnns)
            self.contour_gnn_bns.append(bns_gnn)

            self.contour_gnns1.append(gnns1)
            self.contour_gnn_bns1.append(bns_gnn1)

            self.contour_gnns2.append(gnns2)
            self.contour_gnn_bns2.append(bns_gnn2)

        self.contour_gnns = nn.ModuleList(self.contour_gnns)
        self.contour_gnn_bns = nn.ModuleList(self.contour_gnn_bns)

        self.contour_gnns1 = nn.ModuleList(self.contour_gnns1)
        self.contour_gnn_bns1 = nn.ModuleList(self.contour_gnn_bns1)

        self.contour_gnns2 = nn.ModuleList(self.contour_gnns2)
        self.contour_gnn_bns2 = nn.ModuleList(self.contour_gnn_bns2)
    ...
    def hierarchical_graph_modeling(self, v, layer_idx):
        ...
        for idx in range(v.shape[1]):
            ...
            adj_mat_i = self.cos_sim(part_i_new)
            adj_mat_i = self.normalize(adj_mat_i)
            part_i_new = self.contour_gnns1[layer_idx][idx](part_i, adj_mat_i)
            part_i_new = self.contour_gnn_bns1[layer_idx][idx](part_i_new.transpose(1, 2))
            part_i_new = part_i_new.transpose(1, 2)
            part_i_new = self.relu(part_i_new)

            adj_mat_i = self.cos_sim(part_i_new)
            adj_mat_i = self.normalize(adj_mat_i)
            part_i_new = self.contour_gnns2[layer_idx][idx](part_i, adj_mat_i)
            part_i_new = self.contour_gnn_bns2[layer_idx][idx](part_i_new.transpose(1, 2))
            part_i_new = part_i_new.transpose(1, 2)
            part_i_new = self.relu(part_i_new)
        ...
        adj_mat = self.cos_sim(v_global)
        adj_mat = self.normalize(adj_mat)
        v_global = self.contour_gnns1[layer_idx][-1](v_local, adj_mat)
        v_global = self.contour_gnn_bns1[layer_idx][-1](v_global.transpose(1, 2))
        v_global = v_global.transpose(1, 2)
        v_global = self.relu(v_global)

        adj_mat = self.cos_sim(v_global)
        adj_mat = self.normalize(adj_mat)
        v_global = self.contour_gnns2[layer_idx][-1](v_local, adj_mat)
        v_global = self.contour_gnn_bns2[layer_idx][-1](v_global.transpose(1, 2))
        v_global = v_global.transpose(1, 2)
        v_global = self.relu(v_global)
...
def dim_multigcn_multi50(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = MyModel(
...
def dim_multigcn_multi34(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = MyModel(
        num_classes=num_classes,
        loss=loss,
        block_rgb=Bottleneck,
