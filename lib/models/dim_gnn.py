...
class ResNet(nn.Module):
    ...
    def featuremaps(self, x, x_aux):
        ...
        adj_mat = self.cos_sim(x_part)
        x_part = self.gcn(x_part, adj_mat)
    ...
    def forward(self, x1, x2, return_featuremaps=False):
        ...
        f, f_aux, fpart = self.featuremaps(x, x_aux)
        f_fuse = f * f_aux
        ...
        if not self.training:
            ...
            test_feat11 = torch.cat([test_feat0, test_feat1, test_feat2], dim=1)
            test_feat12 = torch.cat([test_feat0, test_feat1, test_feat2, v_aux], dim=1)
            test_feat13 = torch.cat([test_feat1, v_aux], dim=1)

            return [test_feat0, test_feat1, test_feat2, test_feat4,
                    test_feat7, test_feat10, test_feat11, test_feat12, test_feat13]
        ...
        if self.loss == 'softmax':
            return y, y_aux, y_fuse, ej, em, ypart
        elif self.loss == 'triplet':
            return y, y_aux, y_fuse, ej, em, ypart, \
                   v, v_aux, v_fuse, vpart
        else:
...
"""ResNet"""
def dim_gnn_resnet50(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
