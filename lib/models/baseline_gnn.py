...
class MyModel(nn.Module):
    ...
    def __init__(self, num_classes, loss, block_rgb, layers_rgb, block_contour, layers_contour, zero_init_residual=False,
        ...
        self.gnn = GraphConvolution(self.feature_dim, self.feature_dim_gnn, bias=True)
    ...
    def featuremaps(self, x1, x2):
        ...
        x2 = self.parts_avgpool(x2)
        # print('shape: ', x2.shape)

        # gnn feature embedding
        print('x1:', torch.mean(x1))
        print('x2 before GNN: ', torch.mean(x2))
        x2 = x2.transpose(1, 2)
        x2 = x2.view(x2.size(0), x2.size(1), -1)
        adj_mat = self.cos_sim(x2)
        adj_mat = self.normalize(adj_mat)
        # print(adj_mat)
        x2 = self.gnn(x2, adj_mat)
        x2 = self.bn_gnn(x2.transpose(1, 2))
        x2 = x2.transpose(1, 2)
        x2 = self.relu(x2)

        print('x2 after GNN: ', torch.mean(x2))
        # # # print('After GNN x1: ', torch.max(x1), torch.min(x1), torch.mean(x1))
        # print('\n')

        # x2 = Variable(x2, requires_grad=True)
        # x2.register_hook(lambda grad: print(torch.mean(grad, dim=0)))
    ...
    def forward(self, x1, x2, return_featuremaps=False):
        ...
        # v2 = torch.mean(f2, dim=1)
        # v2 = torch.sum(f2, dim=1)
        v_fuse = self.global_avgpool(f_fuse)
        v_fuse = v_fuse.view(v_fuse.size(0), -1)
        print('The mean of v1 is {}'.format(torch.mean(v1)))
        print('The mean of v2 is {}'.format(torch.mean(v2)))
        print('\n')
        ...
        if self.loss == 'softmax':
            return y1, y2, y_fuse
        elif self.loss == 'triplet' or self.loss == 'triplet_baseline':
            return y1, y2, y_fuse, v1, v2, v_fuse
        else:
...
"""ResNet"""
def my_baseline_gnn(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = MyModel(
    ...
    # print('After Loading Weight', torch.max(model.gnn.weight), torch.min(model.gnn.weight), torch.mean(model.gnn.weight))
