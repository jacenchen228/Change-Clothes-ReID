...
class MyModel(nn.Module):
    ...
    def __init__(self, num_classes, loss, block_rgb, layers_rgb, block_contour, layers_contour, zero_init_residual=False,
        ...
        #                                          feature_dim=1024)
        self.discriminator = GlobalDiscriminator(in_feature_dim=2048, feature_dim=512)
    ...
    def featuremaps(self, x1, x2):
        ...
        x2_part = self.gnn(x2_part, adj_mat)

        x2_part = self.bn_gnn(x2_part.transpose(1, 2))
        x2_part = x2_part.transpose(1, 2)
        x2_part = self.relu(x2_part)

        return x1, x2, x_fuse, x2_part
    ...
    def forward(self, x1, x2, return_featuremaps=False):
        ...
        ej = self.discriminator(v1_new, f2)
        # em = self.discriminator(v1, v2_shuffle)
        em = self.discriminator(v1_new, f2_shuffle)
        # print('ej is ', torch.mean(ej))
        # print('em is ', torch.mean(em))
        # print('\n')
        ######### Mutual Learning version 3 #########
...
"""ResNet"""
def dim_graph_resnet50_C(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = MyModel(
