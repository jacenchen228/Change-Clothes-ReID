...
__all__ = ['dim_part_graph_hie_resnet50_C1']
...
class MyModel(nn.Module):
    ...
    def __init__(self, num_classes, loss, block_rgb, layers_rgb, block_contour, layers_contour, zero_init_residual=False,
        ...
        self.discriminator_part = PartDiscriminator(in_feature_dim=2048, feature_dim=512, vec_feature_dim=2048, part_height=self.part_height)
    ...
    def forward(self, x1, x2, return_featuremaps=False):
        ...
        # v1_dim_g = self.global_avgpool(f1)
        # v1_dim_g = v1_dim_g.view(v1_dim_g.size(0), -1)
        ...
        v1_dim_p = self.parts_avgpool_rgb(f1)
        ej_part = []
        em_part = []
        height_record = 0
        ...
        if self.loss == 'softmax':
            return y1, y2, y_fuse, ej, em, ej_part, em_part
        elif self.loss == 'triplet':
            # v1 = F.normalize(v1, p=2, dim=1)
            # v2 = F.normalize(v2, p=2, dim=1)
            # v_fuse = F.normalize(v_fuse, p=2, dim=1)
            return y1, y2, y_fuse, v1, v2, v_fuse, ej, em, ej_part, em_part
        else:
...
"""ResNet"""
def dim_part_graph_hie_resnet50_C1(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = MyModel(
