...
class Model_2stream(nn.Module):
    def __init__(self, num_classes, batch_size, reduce_dim=768, norm_layer=None, **kwargs):
        ...
        # '''
        # ReID backbone = ResNet 50
        # '''
        # reduce_dim = 768 # for resnet50
        ...
        ReID backbone = PCB
        '''
        reduced_dim_reid = 256
        # self.bn_reid = nn.ModuleList([nn.BatchNorm1d(reduced_dim_reid) for _ in range(self.parts)])
        # self._init_bns(self.bn_reid)
        self.classifier_reid = nn.ModuleList([nn.Sequential(nn.Dropout(), nn.Linear(reduced_dim_reid, num_classes)) for _ in range(self.parts)])
        self._init_fcs(self.classifier_reid)

        '''
        3D bnneck and classifier
        '''
        self.bn3d = nn.BatchNorm2d(reduce_dim3d)
        ...
        # Create ReID backbone
        self.reid_encoder = resnet50_base(num_classes=num_classes, pretrained=True)
        self.dropout = nn.Dropout(p=0.5)
        self.conv5 = DimReduceLayer(512 * 4, reduced_dim_reid, nonlinear='relu')
    ...
    def forward(self, inputs1, inputs2, real_params=None, return_params3D=False):
        ...
        # v_bnneck = self.bn_reid(v_h)
        # v_bnneck = []
        # for i in range(self.parts):
        #     v_bnneck_i = self.bn_reid[i](v_h[:, :, i, :])
        #     v_bnneck.append(v_bnneck_i)
        # v_bnneck = torch.cat(v_bnneck, dim=2)
        ...
        if not self.training:
            ...
            # v_bnneck = F.normalize(v_bnneck, p=2, dim=1)
            # v_bnneck = v_bnneck.view(v_bnneck.size(0), -1)

            # v_concate1 = torch.cat([F.normalize(v_bnneck, p=2, dim=1), F.normalize(v3d_bnneck, p=2, dim=1)], dim=1)
            # v_concate2 = torch.cat([F.normalize(v_h, p=2, dim=1), F.normalize(v3d, p=2, dim=1)], dim=1)
            v_concate1 = torch.cat([v_g, F.normalize(v3d, p=2, dim=1)], dim=1)

            # return [v_concate1, v_concate2, v_bnneck, v_h, v3d_bnneck, v3d]
            return [v_concate1, v_g, v3d_bnneck, v3d]
        ...
        y_reid = []
        for i in range(self.parts):
            # v_bnneck_i = v_bnneck[:, :, i]
            # v_bnneck_i = v_bnneck_i.view(v_bnneck_i.size(0), -1)
            # y_i = self.classifier_reid[i](v_bnneck_i)

            v_h_i = v_h[:, :, i, :]
            v_h_i = v_h_i.view(v_h_i.size(0), -1)
            y_i = self.classifier_reid[i](v_h_i)
...
def model_2stream_part(num_classes=150, loss='softmax', **kwargs):
    model = Model_2stream(
